...
public:
    ...
    /// SS pin is used as chipSelectPin by default.
    NRF24(uint8_t chipEnablePin = 8);
    ...
    /// \return true on success
    bool setStaticPayloadMode(uint8_t size);

    bool setDynamicPayloadMode();
    ...
    /// \return the size of the message, if a complete, valid message has been received and is able to be retrieved by
    /// recv(). Or \return 0.
    uint8_t        available();
    ...
    /// Returns the length of the message received.
    uint8_t waitAndRecv(uint8_t* buf, uint16_t timeout);
